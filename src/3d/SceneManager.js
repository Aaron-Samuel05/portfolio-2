...
export class SceneManager {
  ...
  async init() {
    try {
      let THREE;
      if (window.THREE) THREE = window.THREE; else { const module = await import('https://esm.sh/three@0.160.0'); THREE = module.default || module; }
  ...
  setupScene() {
    ...
    this.scene.fog = new this.THREE.FogExp2(0x000000, .0045);
  }

  setupCamera() {
    const { THREE } = this; const aspect = window.innerWidth / window.innerHeight;
    this.camera = new THREE.PerspectiveCamera(34, aspect, .1, 100);
    this.camera.position.set(0, 1.8, 7.1);
  }

  setupLights() {
    const { THREE } = this;
    const ambient = new THREE.AmbientLight(0xffffff, 1.15); this.scene.add(ambient);
    const hemi = new THREE.HemisphereLight(0xddeeff, 0x050505, 1.2); this.scene.add(hemi);

    const key = new THREE.DirectionalLight(0xffffff, 7.5); key.position.set(4.5, 8, 7); key.castShadow = true;
    key.shadow.mapSize.set(2048, 2048); key.shadow.camera.near = .1; key.shadow.camera.far = 30; key.shadow.bias = -.0004; this.scene.add(key);
    ...
    const top = new THREE.AreaLight ? null : null;
    const kicker = new THREE.PointLight(0xffa45c, 6, 12); kicker.position.set(0, 5, -4); this.scene.add(kicker);
  }

  setupRenderer() {
    const { THREE } = this; this.canvas = document.createElement('canvas'); this.canvas.className = 'webgl-canvas'; this.container.appendChild(this.canvas);
    this.renderer = new THREE.WebGLRenderer({ canvas: this.canvas, alpha: false, antialias: true, powerPreference: 'high-performance' });
    this.renderer.setClearColor(0x000000, 1); this.renderer.setSize(window.innerWidth, window.innerHeight); this.renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
    this.renderer.toneMapping = THREE.ACESFilmicToneMapping; this.renderer.toneMappingExposure = 1.72;
    this.renderer.outputColorSpace = THREE.SRGBColorSpace; this.renderer.shadowMap.enabled = true; this.renderer.shadowMap.type = THREE.PCFSoftShadowMap;
    this.canvas.addEventListener('webglcontextlost', e => { e.preventDefault(); cancelAnimationFrame(this.animationFrameId); this.renderFallback2D(); });
  }

  setupCore() {
    this.core = new KineticCore(this.THREE); this.scene.add(this.core.group);
    this.core.group.traverse(obj => { if (obj.isMesh) { obj.castShadow = true; obj.receiveShadow = true; } });
    const { THREE } = this; const particleCount = 100; const geom = new THREE.BufferGeometry(); const positions = new Float32Array(particleCount * 3);
    for (let i = 0; i < particleCount * 3; i += 3) { positions[i] = (Math.random() - .5) * 18; positions[i + 1] = Math.random() * 9; positions[i + 2] = (Math.random() - .5) * 10 - 2; }
    geom.setAttribute('position', new THREE.BufferAttribute(positions, 3));
    this.particles = new THREE.Points(geom, new THREE.PointsMaterial({ color: 0xffd0a8, size: .018, transparent: true, opacity: .22 })); this.scene.add(this.particles);
  }
  ...
  render() {
    this.animationFrameId = requestAnimationFrame(this.render.bind(this)); const delta = this.clock ? this.clock.getDelta() : .016;
    this.mouseX += (this.targetMouseX - this.mouseX) * .05; this.mouseY += (this.targetMouseY - this.mouseY) * .05;
    if (this.core) this.core.update(delta, this.scrollProgress, this.mouseX, this.mouseY);
    if (this.particles) { this.particles.rotation.y = this.scrollProgress * .35 + this.mouseX * .04; this.particles.rotation.x = this.mouseY * .03; }
    if (this.camera) {
      const targetZ = 7.1 - Math.sin(this.scrollProgress * Math.PI) * .6; const targetX = this.mouseX * .16; const targetY = 1.75 + Math.sin(this.scrollProgress * Math.PI * 2) * .04;
      this.camera.position.z += (targetZ - this.camera.position.z) * .04; this.camera.position.x += (targetX - this.camera.position.x) * .04; this.camera.position.y += (targetY - this.camera.position.y) * .04; this.camera.lookAt(0, 1.8, 0);
    }
